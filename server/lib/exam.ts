...
export function settleExam(tx: ExamTransaction, paper: ExamPaper, now: number, submit = false): ExamPaper {
  ...
  return tx.update(examPapers)
    .set({ status: "completed", currentKey: null, submittedAt: Math.min(now, paper.deadline), score })
...
export function examSummary(paper: ExamPaper, items = paperItems(paper.id)): ExamSummary {
  ...
  const breakdown: ExamKindBreakdown[] = EXAM_KINDS.map(({ kind, total }) => {
  ...
  return {
    ...
    total: 50,
    answered: 50 - unanswered,
    correct, wrong, unanswered,
    score: paper.score,
    fullScore: 100,
    accuracy: Math.round((correct / 50) * 1000) / 10,
    breakdown,
  };
}
